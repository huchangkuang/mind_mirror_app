...
import { submitMbti } from "@/features/mbti/api";
import { useMbtiStore } from "@/features/mbti/store";

export default function MbtiTestScreen() {
  const router = useRouter();
  const { bank, currentIndex, answers, answerQuestion, next, prev, setResult } = useMbtiStore();
  ...
  if (!bank || bank.questions.length === 0) {
    return (
      <View style={styles.container}>
        <Text>题库未加载，请返回 MBTI 介绍页重试。</Text>
      </View>
    );
  }
    ...
    try {
      const payloadAnswers =
        bank.mode === "deep"
          ...
          : answers;
      const result = await submitMbti({
        version: bank.version,
        mode: bank.mode,
        answers: payloadAnswers,
      });
      setResult(result);
      router.replace("/(main)/mbti/result");
    } catch {
      setError("提交失败，请稍后重试");
    } finally {
      setSubmitting(false);
